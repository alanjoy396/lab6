...
![4 letter pairs](4_letter_pairs.png)
...
def generate_graph(words):
    ...
    def edit_distance_one(word):
        for permutation_of_word in list(map("".join, permutations(word))):
            for i in range(len("".join(permutation_of_word))):
                left, c, right = permutation_of_word[0:i], permutation_of_word[i], permutation_of_word[i + 1:]
                j = lookup[c]  # lowercase.index(c)
                for cc in lowercase[j + 1:]:
                    yield left + cc + right
    candgen = ((word, cand) for word in sorted(words)
...
![Unordered Solution](unordered_solution.png)
